...
resource "aws_kms_key" "monitoring" {
  ...
  policy                  = data.aws_iam_policy_document.monitoring_kms.json
...
data "aws_iam_policy_document" "monitoring_kms" {
  statement {
...
locals {
  monitored_tgs = {
    reservas = aws_lb_target_group.reservas.arn_suffix
    pagos    = aws_lb_target_group.pagos.arn_suffix
  }
    ...
    widgets = [
      ...
      },
      {
        ...
        properties = {
          ...
          stat   = "Minimum"
          metrics = [
            ["AWS/ApplicationELB", "HealthyHostCount", "LoadBalancer", aws_lb.main.arn_suffix, "TargetGroup", aws_lb_target_group.reservas.arn_suffix, { label = "reservas" }],
            ["AWS/ApplicationELB", "HealthyHostCount", "LoadBalancer", aws_lb.main.arn_suffix, "TargetGroup", aws_lb_target_group.pagos.arn_suffix, { label = "pagos" }]
          ]
        }
      },
      {
        ...
        properties = {
          ...
          stat   = "Average"
          metrics = [
            ["AWS/ECS", "CPUUtilization", "ClusterName", aws_ecs_cluster.main.name, "ServiceName", aws_ecs_service.reservas.name, { label = "reservas" }],
            ["AWS/ECS", "CPUUtilization", "ClusterName", aws_ecs_cluster.main.name, "ServiceName", aws_ecs_service.pagos.name, { label = "pagos" }]
          ]
        }
      },
      {
        ...
        properties = {
          ...
          stat   = "Average"
          metrics = [
            ["AWS/ECS", "MemoryUtilization", "ClusterName", aws_ecs_cluster.main.name, "ServiceName", aws_ecs_service.reservas.name, { label = "reservas" }],
            ["AWS/ECS", "MemoryUtilization", "ClusterName", aws_ecs_cluster.main.name, "ServiceName", aws_ecs_service.pagos.name, { label = "pagos" }]
          ]
        }
      },
      {
      ...
      },
      {
        ...
        properties = {
          ...
          stat   = "Sum"
          metrics = [
            ["AWS/ApiGateway", "5XXError", "ApiId", aws_apigatewayv2_api.main.id, "Stage", "$default"]
          ]
        }
      },
      {
        ...
        properties = {
          ...
          period = 60
          metrics = [
            ["AWS/ApiGateway", "IntegrationLatency", "ApiId", aws_apigatewayv2_api.main.id, "Stage", "$default", { stat = "p99", label = "p99" }],
            ["AWS/ApiGateway", "IntegrationLatency", "ApiId", aws_apigatewayv2_api.main.id, "Stage", "$default", { stat = "p50", label = "p50" }]
          ]
        }
      }
    ]
