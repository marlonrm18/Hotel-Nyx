...
data "aws_iam_policy_document" "svc_reservas_task" {
  ...
  statement {
    ...
    resources = [
      ...
      "arn:${data.aws_partition.current.partition}:secretsmanager:${var.aws_region}:${data.aws_caller_identity.current.account_id}:secret:${var.project}/${var.environment}/reservas/*",
    ]
  }
...
data "aws_iam_policy_document" "svc_pagos_task" {
  statement {
    ...
    resources = [
      ...
      aws_secretsmanager_secret.rds.arn,
      # Secrets propios del servicio (futuros), bajo su prefijo.
      "arn:${data.aws_partition.current.partition}:secretsmanager:${var.aws_region}:${data.aws_caller_identity.current.account_id}:secret:${var.project}/${var.environment}/pagos/*",
    ]
  }
  ...
  # RDS (aws_kms_key.rds). Restringido a esa clave y al uso via Secrets Manager.
  statement {
    sid       = "KMSDecryptSecrets"
    effect    = "Allow"
    actions   = ["kms:Decrypt", "kms:DescribeKey"]
